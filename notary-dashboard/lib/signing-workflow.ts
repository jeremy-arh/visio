...
export async function loadSigningContext(
  ...
  if (!session) {
    console.warn("[WORKFLOW] ✗ Session not found:", sessionId);
    return null;
  }
  const [{ data: documents }, { data: signers }, { data: notaryRowsFromNotaries }] = await Promise.all([
    supabase
    ...
    supabase
      ...
      .select("id, name, email, order, kyc_status, signed_at")
      .eq("session_id", sessionId)
  ...
  // Fallback : si le notaire n'est pas dans la table "notaries", chercher dans "notary"
  let notaryRows = notaryRowsFromNotaries;
  if (session.notary_id && !(notaryRowsFromNotaries || []).length) {
    const { data: altNotary } = await supabase
      .from("notary")
      .select("id, name, email")
      .eq("id", session.notary_id)
      .limit(1);
    if (altNotary?.length) notaryRows = altNotary;
  }

  let docs = (documents || []) as SessionDocumentRow[];

  // Sessions legacy sans session_documents : bootstrap depuis document_url / submission
  if (allowBootstrap && docs.length === 0 && session.status !== "completed") {
...
export async function advanceSigningWorkflow(
  ...
  /** Le notaire démarre le flux via POST …/start-signing (idle → pending_signers). */
    ...
    for (const doc of context.documents) {
      ...
      if (hasUnsignedNotary || (context.session.notary_id && notarySigs.length === 0)) {
        ...
        await supabase
          ...
          .eq("id", sessionId);
        context = await loadSigningContext(supabase, sessionId);
        break;
      }
    }
  }
  ...
  const allNotariesSignedCheck = notaryRows.length > 0 && notaryRows.every((s) => s.status === "signed");
  ...
  const allNotariesSigned = allNotariesSignedCheck;
